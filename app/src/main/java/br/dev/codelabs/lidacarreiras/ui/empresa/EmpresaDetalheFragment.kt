...
import br.dev.codelabs.lidacarreiras.databinding.FragmentEmpresaDetalheBinding
import dagger.hilt.android.AndroidEntryPoint
import java.time.LocalDateTime

@AndroidEntryPoint
class EmpresaDetalheFragment : Fragment() {
    @RequiresApi(Build.VERSION_CODES.O)
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val viewModel : EmpresaViewModel by activityViewModels()

        val binding = FragmentEmpresaDetalheBinding.inflate(layoutInflater)
